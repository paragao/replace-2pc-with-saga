...
```bash
sam-step-functions$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
sam-step-functions$ sam local start-api
sam-step-functions$ curl http://localhost:3000/
```
...
```bash
sam-step-functions$ sam logs -n HelloWorldFunction --stack-name sam-step-functions --tail
```
...
```bash
sam-step-functions$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name sam-step-functions
```
